...
- 任意两个检查区域（目标/留白）若有**正面积**交叠就**整单拒绝**——同一面不能同时服从两种颜色规则；共边、共点合法。判定直接基于细分面（面的代表点同时落在两个区域内才算重叠），因此同边界不同色、共边半叠、包含关系都会被抓住，且结果与区域排列顺序无关；
- 自交/自触/重边等不合法多边形输入**整单拒绝**（422），不出报告性结论；
- 任何编辑（含项目改名）都会使旧报告**过期**；校样只针对开始时的一致性输入快照，落库时在事务内重校版本——**计算期间发生的编辑会使整份校样作废**（409，不落库、不可确认），旧稿结论绝不会贴到新稿上；
- 通过报告被**确认**时冻结全部输入（项目名、片、幕均不可改），改稿需显式解冻。
...
- `backend/geom`：细缝（整数宽与有理宽）、共边、共点、内环方向、
  ...
  区域重叠（同边界不同色、共边半叠、包含、共点合法、顺序无关）。
- `backend/api`、`backend/store`：完整证明生命周期（过期/冻结/解冻）、
  最早不合格幕、422、旋转、校验错误、410 过期报告、**计算期间编辑→409 且不落库**、
  **冻结后改名/删除阻止、活稿改名使旧报告过期**。
